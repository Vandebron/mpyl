...
- **Python as YAML templating engine** Attempts to mix markup and logic tend to have a
  ...
  outcome. MPyL uses `YAML` strictly for configuration. Where `YAML` needs to be produced dynamically, like for CRD  
  manifests via HELM charts, it is generated by Python code. Te resulting `YAML` is then validated against the corresponding schemas.
...
### Requirements
The following software is expected to be present on the local OS:
