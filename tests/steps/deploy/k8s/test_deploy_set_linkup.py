...
class TestDeploySetLinkup:
    ...
    def test_should_link_up_to_base_services_if_not_pr(self):
        envs = {'KEY_1': 'http://energy-dashboard.{namespace}.svc.cluster.local:4082',
                'KEY_2': 'http://main-website.{namespace}.svc.cluster.local:4050',
                'KEY_3': 'abcd'}
        expected_envs = {'KEY_1': 'http://energy-dashboard.webapps.svc.cluster.local:4082',
                         'KEY_2': 'http://main-website.webapps.svc.cluster.local:4050',
                         'KEY_3': 'abcd'}
        replaced_envs = substitute_namespaces(envs, self.all_projects, self.projects_to_deploy, None)
