...
from src.mpyl.reporting.targets.jira import (
    extract_ticket_from_branch,
    JiraTicket,
    to_markdown_summary,
    to_github_markdown,
    JiraConfig,
    create_jira_for_config,
)
...
class TestJiraReporter:
    ...
    def test_should_print_results_as_string(self):
        ticket_json = json.loads(
            Path(self.test_resource_path / "jira_issue.json").read_text(
                encoding="utf-8"
            )
        )
        ...
        assert ticket.description.startswith(
            "Similarly to mpl *modules*, we _should_ update the PR details with card"
        )

    def test_convert_jira_to_github_markdown(self):
        jira_markdown = Path(self.test_resource_path / "markdown_jira.md").read_text(
            encoding="utf-8"
        )
        github_markdown = to_github_markdown(
            jira_markdown, "http://vandebron.atlassian.com"
        )
        assert_roundtrip(
            self.test_resource_path / "markdown_jira_github.md", github_markdown
        )
    ...
    def test_ticket_to_summary(self):
        ...
        assert_roundtrip(
            self.test_resource_path / "markdown_jira_ticket_to_github.md", summary
        )
    ...
    def test_should_extract_ticket_from_branch(self):
        ...
        assert (
            extract_ticket_from_branch("feature/TICKET-281-slack-reporter", pattern)
            == "TICKET-281"
        )
        assert (
            extract_ticket_from_branch("feature/TECH-281-slack-reporter", pattern)
            == "TECH-281"
        )
        assert (
            extract_ticket_from_branch("feature/ARC-590-slack-reporter", pattern)
            == "ARC-590"
        )
        assert (
            extract_ticket_from_branch("feature/arc-590-slack-reporter", pattern)
        ...
        assert (
            extract_ticket_from_branch("feature/MG-255-slack-reporter", pattern)
            == "MG-255"
        )
        assert (
            extract_ticket_from_branch("feature/mg-255-slack-reporter", pattern)
        ...
        assert (
            extract_ticket_from_branch("chore/arc-590-slack-reporter", pattern)
            == "ARC-590"
        )
        assert (
            extract_ticket_from_branch("chore:arc-590-slack-reporter", pattern)
        ...
        assert (
            extract_ticket_from_branch("arc-590-slack-reporter", pattern) == "ARC-590"
        )
