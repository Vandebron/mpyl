...
class TestDocker(Step):
    ...
    def extract_test_results(
        logger: Logger, project: Project, tag: str, step_input: Input
    ) -> Artifact:
        ...
        if not docker.container.exists(container_id):
            raise ValueError(
                f"Container {container_id} with test results does not exist"
            )
        ...
        try:
            ...
            logger.warning(
                f"Could not find test results in container {tag} at expected location {path_in_container}"
            )
            raise exc
